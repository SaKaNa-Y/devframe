---
title: 'Getting Started'
navigation:
  icon: i-lucide-rocket
description: 'Answer a few questions about the devtool you want to build and get a reading list tailored to it.'
---

Devframe is a framework-neutral foundation for building a devtool once and running it everywhere: inside any host framework, as a standalone tool, or through a coding agent. See the [Introduction](/guide) for the full picture, or read [Pluggable, Extensible, and Playful DevTools](/posts/pluggable-extensible-playful-devtools) for the story behind it.

Devframe provides a lot of features to build DevTools with various needs. It could be overwhelming to start with. Here we have the wizard to help you get started. Answer a few questions about the devtool you want to build and get a reading list tailored to it.

::getting-started-wizard
::

None of this is a required reading order. [`defineDevframe()`](/guide/devframe-definition) and [the tutorial](/guide/tutorial-server-data-inspector) are worth reading regardlessly.
