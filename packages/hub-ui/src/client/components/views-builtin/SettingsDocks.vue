...
const categories = computed<DevframeDockEntriesGrouped>(() => {
...
function membersOf(groupId: string): DevframeDockEntry[] {
  return getGroupMembers(props.context.docks.entries, groupId, settings.value, {
    includeHidden: true,
  })
}

function subcategoriesOf(groupId: string): DevframeDockEntriesGrouped {
...
function itemsOfContainer(container: string): DevframeDockEntry[] {
...
function defaultItemsOfContainer(container: string): DevframeDockEntry[] {
